(function(){
  ...
  function isManifestSource(c){return /(?:^|[.!?]\s*)Manifest\s*[—-]/i.test(String(c?.text||''));}
  function breakdown(p,c){
    ...
    const nextRelic=c?.type==='Relic'?Math.max(0,Number(v.nextRelicDiscount)||0):0;
    const nextGuardian=c?.type==='Guardian'?Math.max(0,Number(v.nextGuardianDiscount)||0):0;
  ...
  function decorate(){
    ...
    const ctx=typeof activeCardContext!=='undefined'?activeCardContext:null,btn=document.getElementById('detailPlay');
